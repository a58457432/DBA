...
class DBStatus(object):
    ...
    def __init__(self):
        ...
        self.global_status = ('KEY_READS', 'KEY_READ_REQUESTS', 'KEY_WRITES', 'KEY_WRITE_REQUESTS', 'KEY_BLOCKS_UNUSED',
                              ...
                              'CREATED_TMP_DISK_TABLES', 'SLOW_QUERIES', 'THREADS_CONNECTED','THREADS_RUNNING',
...
#                     WHERE a.VARIABLE_NAME in %s""" % str(self.global_status)
            ...
            try:
                self.cur = self.conn.cursor(MySQLdb.cursors.DictCursor)
                sql = "select info,time,db,state from information_schema.processlist where command != 'Sleep' and user != 'system user'  order by TIME desc limit 10;"
                self.cur.execute(sql)
                rows = self.cur.fetchall()
                db['processlist'] = list(rows)
    ...
    def cal_cur_status(self):
        ...
        for db in self.dbins.values():
            ...
            try:
                ...
                #当前线程并发
                db['cur_status_cal']['Threads_running'] = int(db['global_status']['THREADS_RUNNING'])
    ...
    def write_result_file(self):
        ...
        with open(DBSTATUS_FILE, 'w') as f:
            ...
            f.write(insname + "." + 'processlist' + "=" + str(dbs[0]['processlist']) + "\n")
            f.write("\n\n")
